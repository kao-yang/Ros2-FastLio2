...
public:
    ...
    /*
    * 对应地图V-B节的Data Struct
    */
    struct KD_TREE_NODE
    {
        PointType point;                        // 节点存储的点
        int division_axis;                      // 分割轴
        int TreeSize = 1;                       // 根值于当前节点的子树的节点数，包含有效点与无效点
        int invalid_point_num = 0;              // 子树无效点
        int down_del_num = 0;
        bool point_deleted = false;             // 表明该节点待删除
        bool tree_deleted = false;              // 表面删除了该节点及其子树
...
{
    if (Root_Node != nullptr)           // 如果有Root，需要删除之前有的Root
    {
        delete_tree_nodes(&Root_Node);
    }
    if (point_cloud.size() == 0)        // 如果输入点数量为0，结束
        return;
    STATIC_ROOT_NODE = new KD_TREE_NODE;
    InitTreeNode(STATIC_ROOT_NODE);     // 初始化ikd树
    BuildTree(&STATIC_ROOT_NODE->left_son_ptr, 0, point_cloud.size() - 1, point_cloud); // 构建ikd树，实际构建的是左子树
    Update(STATIC_ROOT_NODE);           // 更新root节点属性
    STATIC_ROOT_NODE->TreeSize = 0;     // 
    Root_Node = STATIC_ROOT_NODE->left_son_ptr; // 直接将左子树替换成根节点
}
